...
fn resolve_config_path() -> PathBuf {
    if let Some(dir) = std::env::var_os("NH_CONFIG_DIR") {
    ...
    if let Some(config_dir) = dirs::config_dir() {
        let dir = config_dir.join("nehme-harness");
        let toml = dir.join("config.toml");
        let json = dir.join("config.json");
        if toml.exists() {
            return toml;
...
fn default_quick_models() -> HashMap<String, QuickModelConfig> {
    ...
    map.insert(
        "deepseek-direct-flash".to_string(),
        QuickModelConfig {
            provider: CompactString::new("deepseek"),
            model: CompactString::new("deepseek-v4-flash"),
            input_token_cost: 0.14,
            output_token_cost: 0.28,
            cached_input_token_cost: Some(0.0028),
            reserve_tokens: None,
            temperature: None,
        },
    );
    map.insert(
        "deepseek-direct-pro".to_string(),
        QuickModelConfig {
            provider: CompactString::new("deepseek"),
            model: CompactString::new("deepseek-v4-pro"),
            input_token_cost: 0.435,
            output_token_cost: 0.87,
            cached_input_token_cost: Some(0.14),
            reserve_tokens: None,
            temperature: None,
        },
    );
