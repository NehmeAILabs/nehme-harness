...
pub(crate) fn default_model_for_provider(
    provider: &str,
    cfg: &Config,
) -> Option<(String, Option<(f64, f64, Option<f64>)>)> {
...
impl AnyClient {
    ...
    pub async fn list_models(&self) -> anyhow::Result<Vec<ModelEntry>> {
        let list = match self {
            ...
            AnyClient::DeepSeek(c) => c.list_models().await?,
...
impl AnyAgent {
    pub async fn run_print(
        ...
        max_turns: usize,
        pure_stdout: bool,
    ) -> anyhow::Result<crate::agent::runner::PrintResult> {
        match self {
...
/// Like `create_client`, but instead of failing when no API key is found,
/// uses a placeholder key so the TUI can start. Returns `(client, had_real_key)`.
/// When `had_real_key` is false, the user should be prompted to set a real key
/// via `/provider <name> <key>`.
pub fn create_client_or_fallback(
    provider_name: &str,
    api_key: Option<&str>,
    custom_providers: &HashMap<String, CustomProviderConfig>,
    config_api_keys: Option<&HashMap<String, String>>,
) -> anyhow::Result<(AnyClient, bool)> {
    let config = resolve_provider_config(provider_name, custom_providers)?;
    let base_url = resolve_base_url(&config);
    ...
    let resolver = AuthResolver::new(config.kind)
        ...
        .with_config_keys(config_api_keys)
        .with_custom_provider_name(Some(provider_name));
    let (key, had_real_key) = resolver.resolve_or_placeholder();
    ...
    let client = match config.kind {
        ProviderKind::OpenAI => {
            ...
            let http_client =
                build_http_client(provider_name, config.danger_accept_invalid_certs, custom)?;
            AnyClient::OpenAI(build_openai_client(
                &key,
                base_url.as_deref(),
                custom,
                http_client,
...
fn build_openrouter_client(key: &str, base_url: Option<&str>) -> anyhow::Result<AnyClient> {
    ...
    let builder = builder
        .with_app_identity(
            "nehme-harness",
            "https://github.com/NehmeAILabs/nehme-harness",
        )
...
fn build_deepseek_client(key: &str, base_url: Option<&str>) -> anyhow::Result<AnyClient> {
    build_provider_client!(deepseek::Client, DeepSeek, key, base_url)
}
