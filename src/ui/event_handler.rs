...
pub async fn handle_agent_event(
    ...
    match event {
        ...
        AgentEvent::UsageUpdate { .. } => {
            // Per-call usage updates are informational; the final aggregated
            // usage is applied via AgentEvent::Done. No action needed here.
        }
        AgentEvent::Done {
            response,
            input_tokens,
...
async fn handle_agent_done(
    ...
    *was_reasoning = false;
    ...
    session.total_cost += crate::pricing::estimate_cost_detailed(
        input_tokens,
        output_tokens,
        cached_input_tokens,
