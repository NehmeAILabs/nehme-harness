...
pub async fn handle_compress(
    ...
    #[cfg(feature = "context")]
    {
        let snapshot = crate::extras::context::extract_snapshot(&session.messages[..cut_idx]);
        if !snapshot.is_empty() {
            let data_dir = crate::session::storage::data_dir();
            if let Ok(store) = crate::extras::context::ContextStore::open(&data_dir) {
                let _ = store.save_compact_snapshot(&session.id, &snapshot);
            }
        }
        crate::extras::context::record_event(
            &session.id,
            crate::extras::context::event::COMPACTION,
            crate::extras::context::event::PRIORITY_CRITICAL,
            "slash",
            "manual_compress",
            &format!(
                "{{\"messages_summarized\":{},\"tokens_before\":{}}}",
