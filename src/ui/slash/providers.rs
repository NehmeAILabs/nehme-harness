...
async fn handle_provider(parts: &[&str], ctx: &mut SlashCtx<'_>) -> anyhow::Result<()> {
    ...
    // /provider <name> <key> — set API key and switch provider
    let api_key_arg = parts.get(2).map(|s| s.trim()).filter(|s| !s.is_empty());
    if let Some(key) = api_key_arg {
        let slug = api_key_config_name(new_provider, ctx.cfg);

        // Persist the key to config
        if let Some(ref mut api_keys) = ctx.cfg.api_keys {
            api_keys.insert(slug.clone(), key.to_string());
        } else {
            let mut map = std::collections::HashMap::new();
            map.insert(slug.clone(), key.to_string());
            ctx.cfg.api_keys = Some(map);
        }
        if let Err(e) = crate::config::save_config(ctx.cfg) {
            write_error(ctx.renderer, format!("failed to save API key: {}", e));
        } else {
            write_ok(ctx.renderer, format!("saved API key for '{}'", slug));
        }
    }
    ...
    if let Some((model, costs)) = crate::provider::default_model_for_provider(new_provider, ctx.cfg)
    {
        ...
        if let Some((inc, outc, cachedc)) = costs {
            ctx.session.input_token_cost = inc;
            ctx.session.output_token_cost = outc;
            ctx.session.cached_input_token_cost = cachedc;
        }
    }
...
fn api_key_config_name(provider: &str, cfg: &Config) -> String {
    if cfg.custom_providers_map().contains_key(provider) {
        provider.to_string()
    } else if let Some(kind) = crate::provider::parse_provider(provider) {
        crate::auth::AuthResolver::new(kind)
            .provider_slug()
            .to_string()
    } else {
        provider.to_string()
...
async fn handle_models(parts: &[&str], ctx: &mut SlashCtx<'_>) -> anyhow::Result<()> {
    ...
    if parts.len() >= 2 && !refresh {
        ...
        if let Some(q) = qm.get(arg) {
            ...
            ctx.session.cached_input_token_cost = q.cached_input_token_cost;
            write_result(
                ctx.renderer,
                format!(
...
async fn model_for_subagent(
    ...
    let max_turns = ctx.cfg.task_max_turns.unwrap_or(12);
...
mod tests {
    ...
    fn custom_provider() -> CustomProviderConfig {
        CustomProviderConfig {
            provider_type: compact_str::CompactString::new("openai"),
    ...
    #[test]
    fn api_key_config_name_uses_custom_provider_name() {
        let mut cfg = Config::default();
        cfg.custom_providers = Some(std::collections::HashMap::from([(
            "work-gateway".to_string(),
            custom_provider(),
        )]));

        assert_eq!(api_key_config_name("work-gateway", &cfg), "work-gateway");
    }

    #[test]
    fn api_key_config_name_uses_builtin_slug() {
        let cfg = Config::default();

        assert_eq!(api_key_config_name("openai", &cfg), "openai");
        assert_eq!(api_key_config_name("google", &cfg), "gemini");
    }
}
