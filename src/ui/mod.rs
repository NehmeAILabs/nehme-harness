...
pub async fn run_interactive(
    ...
    if let Some(provider) = missing_api_key {
        ...
        renderer.write_line(
            &format!(
                "No API key set for '{}'. Set {} or use: /provider {} <key>",
    ...
    loop {
        tokio::select! {
            ...
            Some(bev) = btw_rx.recv() => {
                ...
                match bev {
                    crate::event::BtwEvent::Done { id, response, input_tokens, output_tokens, cached_input_tokens } => {
                        btw_total_cost += crate::pricing::estimate_cost_detailed(
