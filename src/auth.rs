...
impl AuthResolver {
    ...
    /// Resolve the API key, returning a helpful error message if not found.
    pub fn resolve(&self) -> anyhow::Result<String> {
        self.resolve_with_env(|name| std::env::var(name))
    }

    /// Resolve the API key, returning a placeholder if not found.
    /// The placeholder allows client construction to succeed so the TUI can start,
    /// but actual API calls will fail with an auth error until the key is set.
    /// Returns Ok((key, had_key)) where had_key indicates whether a real key was found.
    pub fn resolve_or_placeholder(&self) -> (String, bool) {
        match self.resolve() {
            Ok(key) => (key, true),
            Err(_) => (format!("sk-no-key-set-{}", self.provider_slug()), false),
        }
    }
