...
async fn main() -> anyhow::Result<()> {
    ...
    if let Some(qm) = cli.resolve_quick_model(&cfg) {
        ...
        session.cached_input_token_cost = qm.cached_input_token_cost;
    ...
    #[cfg(feature = "context")]
    {
        ...
        if let Err(e) = crate::extras::context::init_store(&data_dir) {
            tracing::warn!("Failed to initialize context store: {e}");
        }
        if let Ok(store) = crate::extras::context::ContextStore::open(&data_dir) {
            if let Ok(Some(snapshot)) = store.load_compact_snapshot(&session.id) {
                context.compact_snapshot = Some(snapshot);
            }
        }
    }

    let (client, has_api_key) = provider::create_client_or_fallback(
        &provider,
    ...
    {
        let task_max_turns = cfg.task_max_turns.unwrap_or(12);
    ...
    if !has_api_key && !is_interactive {
        anyhow::bail!(
            "No API key found for '{}'. Set the {} environment variable, or run interactively to set a key with /provider.",
            provider,
    ...
    if cli.print {
        ...
        if msg.starts_with('!') {
            ...
            let cost = crate::pricing::estimate_cost_detailed(
                result.input_tokens,
                result.output_tokens,
                result.cached_input_tokens,
                session.input_token_cost,
                session.output_token_cost,
                session.cached_input_token_cost,
            );
            if result.cached_input_tokens > 0 {
                eprintln!(
                    "STATS input_tokens={} output_tokens={} cached_input_tokens={} cost=${:.4}",
                    result.input_tokens, result.output_tokens, result.cached_input_tokens, cost
                );
            } else {
                eprintln!(
                    "STATS input_tokens={} output_tokens={} cost=${:.4}",
                    result.input_tokens, result.output_tokens, cost
                );
            }
            if !cli.no_session {
                session.add_message(MessageRole::User, &msg);
                session.add_message(MessageRole::Assistant, &result.text);
                ...
                session.total_cached_input_tokens = session
                    ...
                    .saturating_add(result.cached_input_tokens);
                session.total_cost = session.total_cost + cost;
                session::storage::save_session(&session)?;
                let _ =
...
async fn run_headless_loop(
    ...
    loop {
        ...
        let summary: String = print_result
            .text
            .chars()
