...
fn test_plan_exists_on_nonexistent() {
    let tmp = std::env::temp_dir().join("nehme-harness_test_plan_nonexistent.md");
...
fn test_plan_exists_on_existing() {
    let tmp = std::env::temp_dir().join("nehme-harness_test_plan_exists.md");
...
fn test_read_plan_returns_content() {
    let tmp = std::env::temp_dir().join("nehme-harness_test_read_plan.md");
...
fn test_read_plan_nonexistent_returns_none() {
    let tmp = std::env::temp_dir().join("nehme-harness_test_read_nonexistent.md");
    let _ = std::fs::remove_file(&tmp);
...
fn test_delete_plan_removes_file() {
    let tmp = std::env::temp_dir().join("nehme-harness_test_delete_plan.md");
...
fn test_delete_plan_nonexistent_does_not_panic() {
    let tmp = std::env::temp_dir().join("nehme-harness_test_delete_nonexistent.md");
    let _ = std::fs::remove_file(&tmp);
...
async fn test_handle_startup_no_plan_returns_false() {
    let tmp = std::env::temp_dir().join("nehme-harness_test_startup_nonexistent.md");
    let _ = std::fs::remove_file(&tmp);
...
fn test_transcript_dir_contains_session_id() {
    ...
    let dir = dirs::data_dir()
        .unwrap_or_else(|| PathBuf::from(".nehme-harness"))
        .join("nehme-harness")
        .join("loops")
