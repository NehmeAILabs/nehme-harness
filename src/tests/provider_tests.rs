...
fn expand_env_reads_var() {
    unsafe { std::env::set_var("NH_TEST_HDR", "secret-value") };
    assert_eq!(expand_env("${NH_TEST_HDR}").unwrap(), "secret-value");
    unsafe { std::env::remove_var("NH_TEST_HDR") };
}

#[test]
fn expand_env_missing_var_errors() {
    assert!(expand_env("${NH_DEFINITELY_NOT_SET_98237}").is_err());
}
