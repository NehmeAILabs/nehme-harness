...
pub async fn build_agent_inner<M: CompletionModel + 'static>(
    ...
    #[cfg(feature = "context")]
    if let Some(ref snapshot) = context.compact_snapshot {
    ...
    if cli.resolve_no_tools(cfg) {
        ...
        let mut base_tools: Vec<Box<dyn rig::tool::ToolDyn>> = vec![
            ...
            Box::new(tools::WebSearchTool::new(
                permission.clone(),
                ask_tx.clone(),
                cfg.max_web_search_results.unwrap_or(20),
            )),
            Box::new(tools::WebFetchTool::new(
                permission.clone(),
                ask_tx.clone(),
                cfg.max_web_fetch_length.unwrap_or(200_000),
                session_id.clone(),
            )),
            Box::new(tools::CodeGraphTool::new(
                permission.clone(),
                ask_tx.clone(),
            )),
        ];
        ...
        base_tools.extend([
            ...
            Box::new(crate::extras::context::CtxRetrieveTool::new(
                permission.clone(),
                ask_tx.clone(),
                session_id.clone(),
            )) as Box<dyn rig::tool::ToolDyn>,
            Box::new(crate::extras::context::CtxStatsTool::new(
                permission.clone(),
                ask_tx.clone(),
                session_id.clone(),
            )) as Box<dyn rig::tool::ToolDyn>,
        ...
        if cfg.task_enabled.unwrap_or(true) {
            ...
            builder = builder.tool(TaskTool::new(
                permission.clone(),
                ask_tx.clone(),
                session_id.clone(),
            ));
...
快速旁助手。用户按了/btw，并行提问，不打断主助手。

职责：直接、简短回答用户问题。用对话上下文和项目背景。

长度匹配问题：问候/感谢/是=一行。不主动提供构建/运行说明，除非被问。

只读工具：read, grep, find_files, list_dir, ctx_search, ctx_execute, web_search, web_fetch, code_graph。不能写文件、运行命令、修改任何东西。回复不保存到主对话。不计划主任务，不让用户等主助手。

推理用中文(中文)。最终回答用用户语言。代码、文件路径、变量名、错误信息保持原文。

回复风格：最少token。省略冠词、填充词、客套话。片段式OK。不叙述工具调用。仅在片段可能误解时展开。";
