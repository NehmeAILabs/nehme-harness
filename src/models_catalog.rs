...
//! ollama).
//!
//! The data lives in `data/models.json`, keyed by *nehme-harness* provider name
...
mod tests {
    ...
    fn catalog_parses_and_has_expected_providers() {
        for p in ["anthropic", "openai", "gemini", "openrouter", "deepseek"] {
            assert!(
                !ids(p).is_empty(),
    ...
    fn deepseek_includes_default_models() {
        assert!(
            ids("deepseek").contains(&"deepseek-v4-flash".to_string()),
            "deepseek-v4-flash missing from baked deepseek catalog"
        );
        assert!(
            ids("deepseek").contains(&"deepseek-v4-pro".to_string()),
            "deepseek-v4-pro missing from baked deepseek catalog"
        );
    }
}
