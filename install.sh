#!/usr/bin/env bash
#
# Install nehme-harness from GitHub Releases.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/NehmeAILabs/nehme-harness/main/install.sh | bash
#
#   # Custom install directory:
#   curl -fsSL https://raw.githubusercontent.com/NehmeAILabs/nehme-harness/main/install.sh | bash -s -- --dir /usr/local/bin
#
set -euo pipefail

REPO="NehmeAILabs/nehme-harness"
...
echo "Downloading nh latest (${TARGET})..."
echo "  -> ${URL}"
...
if [[ -f "${TMPDIR}/nh" ]]; then
    cp "${TMPDIR}/nh" "${INSTALL_DIR}/nh"
elif [[ -f "${TMPDIR}/${TARGET}" ]]; then
    cp "${TMPDIR}/${TARGET}" "${INSTALL_DIR}/nh"
else
    # search for any nh binary in the extracted tree
    BIN="$(find "${TMPDIR}" -type f -name nh -o -name "nh-*" | head -1)"
    if [[ -z "$BIN" ]]; then
        echo "Error: could not find nh binary in the archive." >&2
        exit 1
    fi
    cp "$BIN" "${INSTALL_DIR}/nh"
fi

chmod +x "${INSTALL_DIR}/nh"

echo "Installed nh to ${INSTALL_DIR}/nh"
