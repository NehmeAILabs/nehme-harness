...
nehme-harness supports six built-in providers and allows custom provider
...
`NH_PROVIDER` environment variable:

```
nh --provider anthropic
```

The model is set with `--model` or `NH_MODEL`:

```
nh --provider openai --model gpt-4o
```

## Custom Providers

Custom providers let you point nehme-harness at any OpenAI-compatible API (vLLM,
...
| ----------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider_type`               | string  | Must be one of the built-in provider types (`openrouter`, `openai`, `anthropic`, `gemini`, `ollama`, `deepseek`).                                                             |
...
| `model`                       | string  | Optional. Default model name for this provider. Used when no model is specified via `--model` or `NH_MODEL`.                                                                  |
...
nehme-harness enables prompt caching automatically where the underlying rig provider supports it. The behavior depends on which provider backs the model you choose.

### Automatic — no nehme-harness action
...
- **DeepSeek** — automatic prefix caching, persistent across days, ~98% discount on cache hits. Cache hits reported in `usage.prompt_cache_hit_tokens`.
- **xAI / Grok** — automatic, 75% discount; benefits from setting an `x-grok-conv-id` header which nehme-harness does not currently send.
- **Moonshot, Groq (Kimi K2)** — automatic.

For these providers, nehme-harness passes through to rig without additional configuration.

### Explicitly enabled by nehme-harness

These providers require `cache_control` markers; nehme-harness adds them via rig's `.with_prompt_caching()`:
...
- **Claude via OpenRouter** — marks the system prompt only. Anthropic's caching is prefix-based, so the tools array in front of the system block is also captured. For `anthropic/*` model IDs, nehme-harness also pins `provider.order = ["Anthropic"]` with `allow_fallbacks: true`, because Bedrock and Vertex AI silently drop `cache_control` markers.

### Empirical impact

Measured on Sonnet 4.6, second turn of a tool-heavy session (grep + read across the nehme-harness repo, ~6k-token system prompt including AGENTS.md and ARCHITECTURE.md):
...
This is an upstream rig limitation, not nehme-harness-specific.
...
| ------------------- | ------------- | ----------------------------------- |
| `--provider`        | `NH_PROVIDER` | Provider name                       |
| `--model`           | `NH_MODEL`    | Model name                          |
