...
| `/prompt <name>` | Activate a named prompt. Also applies `%%mode=` from the prompt file if present. |
...
`standard`, `restrictive`, `readonly`, `guarded`, `yolo`, `planwrite`.
Use `%%mode=last_user_mode` to restore the mode the user last set via `/mode`.

## Conversation

| Command | Description |
| ------- | ----------- |
| `/compress [instructions]` | Compress conversation history to free context window space. Also saves a compact snapshot to the context store (if context feature enabled). |
...
| `/btw <message>` | Ask a quick side question in parallel, without touching the main conversation. Forks the current context, answers using read-only tools + context tools, prints the answer inline. Nothing is written to history. Ctrl-C cancels without disturbing the main agent. |
| `/reasoning` | Toggle LLM reasoning on/off (requires model support). |
| `/thinking` | Alias for `/reasoning`. |
| `/review [msg]` | Run a one-shot code review. Activates the `review` prompt in readonly mode, submits a review message, and restores the previous prompt afterward. |
...
| Command | Description |
| ------- | ----------- |
| `/model-subagent` | Show the current subagent provider and model. |
| `/model-subagent <name>` | Switch the subagent to a different model (same provider). |
| `/models-subagent` | List quick models. |
| `/models-subagent <name>` | Switch subagent to a named quick model's provider + model. |
...
| `.ask` | Switch to the `ask` prompt. |
| `.plan what files changed?` | Temporarily use the `plan` prompt for this query, then restore. |
