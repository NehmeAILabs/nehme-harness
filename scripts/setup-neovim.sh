#!/usr/bin/env bash
#
# setup-neovim.sh — Configure Neovim with CodeCompanion.nvim + nehme-harness ACP
#
# Usage:
#   bash setup-neovim.sh
#
#   # Skip nehme-harness install check:
#   bash setup-neovim.sh --skip-nh
#
set -euo pipefail

SKIP_NEHME-HARNESS=false
while [[ $# -gt 0 ]]; do
    case "$1" in
        --skip-nh)
            SKIP_NEHME-HARNESS=true
            shift
            ;;
        --help|-h)
            cat <<EOF
Usage: setup-neovim.sh [--skip-nh]

Options:
  --skip-nh   Skip the nehme-harness installation check
...
echo "=== nehme-harness + Neovim ACP Setup ==="
...
# ---- 3. Check nehme-harness ----
if ! $SKIP_NEHME-HARNESS; then
    if command -v nh &>/dev/null; then
        echo "Found nh at $(which nh)"
        if ! nh --help 2>&1 | grep -q '\-\-acp'; then
            echo "WARNING: This nehme-harness build does not include ACP support." >&2
            echo "  Rebuild with: cargo install nehme-harness --features acp" >&2
        fi
    else
        echo "nh not found in PATH." >&2
        echo "  Install from source: cargo install nehme-harness --features acp" >&2
        echo "  Or run with --skip-nh to skip this check." >&2
        exit 1
    fi
fi
        ...
        require("codecompanion").setup({
            adapters = {
                acp = {
                    ...
                    nehme-harness = function()
                        return {
                            name = "nehme-harness",
                            formatted_name = "Nehme-Harness",
                            type = "acp",
                            roles = {
                                llm = "assistant",
                                user = "user",
...
echo "  3. Select the 'nehme-harness' adapter in the chat buffer"
echo ""
echo "Ensure nehme-harness is built with ACP support:"
echo "  cargo install nehme-harness --features acp"
echo ""
