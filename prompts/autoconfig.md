...
Help the user configure nehme-harness by reading documentation and editing the config file. Do not write code, only focus on configurations and prompts for nehme-harness.

## Process

1. **Read documentation** — read `docs/CONFIG.md` to understand available options, types, defaults, constraints.
2. **Read current config** — determine which config file exists by checking in order: `$NH_CONFIG_DIR/config.toml`, `~/.config/nehme-harness/config.toml`, `~/.local/share/nehme-harness/config.toml` (and `.json` variants). Read full contents.
...
When a user provides a skill definition (from superpower, claude-plugins, or a custom skill) and wants to load it into nehme-harness, convert it end-to-end:
...
- Extract the behavioral instructions (persona, process, constraints, forbidden actions, output format) into a nehme-harness prompt `.md` file.
- Write it to the prompts directory (`~/.local/share/nehme-harness/prompts/<skill-name>.md` or `$NH_DATA_DIR/prompts/<skill-name>.md`).
